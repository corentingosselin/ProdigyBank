...
public class Cuboid {
    ...
    public static Cuboid setupCuboid(Player admin) {
        WorldEditPlugin worldEdit = (WorldEditPlugin) Bukkit.getServer().getPluginManager().getPlugin("WorldEdit");
        String msg = ProdigyBank.getInstance().getLanguage().worldedit_no_selection;
        Region selection = null;
        try {
            selection = worldEdit.getSession(admin).getSelection(BukkitAdapter.adapt(admin.getWorld()));
        ...
        Location minimumPoint = BukkitAdapter.adapt(admin.getWorld(), selection.getMinimumPoint());
        Location maximumPoint = BukkitAdapter.adapt(admin.getWorld(), selection.getMaximumPoint());
        return new Cuboid(minimumPoint,maximumPoint);
    }

    public static boolean checkForAir(Cuboid cuboid, Player admin) {
        boolean foundAir = false;
        Iterator<Block> it = cuboid.blockList();
        while(it.hasNext()) {
            Block b = it.next();
        ...
        if(!foundAir) admin.sendMessage(ProdigyBank.getInstance().getLanguage().need_air);
        return foundAir;
    }
}